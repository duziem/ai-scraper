...
def scrape_twitter_mentions(query: str = "Branch OR @BranchApp", limit: int = 100) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # Create realistic simulated data up to requested limit
        for i in range(limit):
            # Generate unique tweet ID with retry logic
            max_retries = 5
            for retry in range(max_retries):
                tweet_id = str(random.randint(1000000000000000000, 9999999999999999999))
                if tweet_id not in collected_ids:
                    break
            else:
                # If all retries failed, create a time-based unique ID
                tweet_id = f"sim_tw_{int(time.time() * 1000)}_{i}"
